...
export async function syncFinalOrderTelegramState(params: {
  ...
  if (config.telegram.chatIdsOrderStatus) {
  ...
  for (const target of notifyTargets) {
    ...
    try {
      ...
      logError(
        params.logger,
        { err: error, chatId: target.chatId, messageId: target.messageId, orderId: context.order.id },
        "Failed to edit cancelled Telegram order message; falling back to send",
      );
      ...
      try {
        ...
        logError(
          params.logger,
          { err: sendError, chatId: target.chatId, orderId: context.order.id },
          "Failed to send cancelled Telegram order message fallback",
        );
      }
    }
  }

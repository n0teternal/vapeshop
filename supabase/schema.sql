...
-- Stock allocated to individual staff members. City inventory remains the
-- customer-facing total; this table answers who physically has each item.
...
create table if not exists public.orders (
  ...
  seller_id bigint references public.staff_members (id) on delete set null,
...
-- Only finite (`inventory.stock_qty`) stock is reserved.  Keeping a record of
-- that reservation prevents a cancellation from returning stock that was never
...
-- Non-sale stock operations are kept outside orders so revenue reports remain accurate.
...
-- Existing deployments may have been created before `inbound` was introduced.
...
-- Private restore points made before product-table imports and image uploads.
-- The API enforces the owner account; there are no public table policies.
